...
export default function UserForm({ onSubmit, onCancel }: UserFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
	...
	companyId: companyId, // Adicionar o companyId
	}

      delete createData.licenseNumber
      delete createData.licenseCategory
      delete createData.licenseExpiration

      if (formData.role === "DRIVER") {
        // Only add license fields if they have values
        if (formData.licenseNumber && formData.licenseNumber.trim() !== "") {
          createData.licenseNumber = formData.licenseNumber.trim()
        }

        if (formData.licenseCategory && formData.licenseCategory.trim() !== "") {
          createData.licenseCategory = formData.licenseCategory.trim()
        }

        // Only include licenseExpiration if it has a value and is valid
        if (formData.licenseExpiration && formData.licenseExpiration.trim() !== "") {
          try {
            const date = new Date(formData.licenseExpiration)
            if (!isNaN(date.getTime())) {
              // Send as ISO string as expected by backend
              createData.licenseExpiration = date.toISOString()
              console.log("[v0] License expiration formatted:", createData.licenseExpiration)
            } else {
              console.log("[v0] Invalid date, skipping licenseExpiration")
            }
          } catch (error) {
            console.error("[v0] Error formatting date:", error)
          }
        } else {
          console.log("[v0] No license expiration provided, skipping field")
        }
      }

      console.log("[v0] Final payload being sent:", JSON.stringify(createData, null, 2))
...
if (formData.role === "WORKSHOP_MANAGER" && workshopId) {
  try {
    await fetchClientSide("PATCH", `/workshops/${workshopId}/manager`, {
      managerId: response.id,
    })
    console.log("[v0] Manager linked to workshop successfully")
  } catch (linkError) {
    console.error("[v0] Error linking manager to workshop:", linkError)
    setError("Usuário criado com sucesso, mas houve erro ao vincular à oficina. Por favor, faça isso manualmente.")
    setTimeout(() => {
      onSubmit(response)
    }, 3000)
    return
...
if (formData.role === "DRIVER" && vehicleId) {
  try {
    await fetchClientSide("PATCH", `/vehicles/${vehicleId}/driver`, {
      driverId: response.id,
    })
    console.log("[v0] Driver linked to vehicle successfully")
  } catch (linkError) {
    console.error("[v0] Error linking driver to vehicle:", linkError)
    setError("Usuário criado com sucesso, mas houve erro ao vincular ao veículo. Por favor, faça isso manualmente.")
    setTimeout(() => {
      onSubmit(response)
    }, 3000)
    return
      ...
      if (error.message) {
        ...
        } else if (message.includes("user_email_key")) {
          errorMessage = "Este email já está sendo usado por outro usuário. Por favor, use um email diferente."
        } else if (message.includes("internal server error")) {
          errorMessage = "Erro interno do servidor. Verifique se o email já não está sendo usado."
        } else {
