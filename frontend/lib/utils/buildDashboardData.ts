...
export function buildDashboardData({
  ...
  const enrichedOrders = orders.map((item) => {
    ...
    const driverName = vehicles.find((v) => v.id === item.vehicleId)?.driver
      .name;
  ...
  return {
    ...
    vehiclesInMaintenance: vehicles.filter((v) => v.status === "MAINTENANCE")
      .length,
    ...
    _orderGroupedByType: groupAndAggregate(enrichedOrders, ["type"], {
      ...
      avgDuration: (items) =>
        avg(items.map((i) => i.durationDiff).filter(Boolean)),
    }),
    ...
    _costByBranch: groupAndAggregate(
      enrichedOrders,
      ["branchName", "branchId"],
      {
        totalCost: (items) => sum(items.map((i) => i.totalCost)),
