...
export default function WorkshopDetailPage({
  ...
  useEffect(() => {
    const fetchWorkshop = async () => {
      try {
        const data = await fetchClientSide<Workshop>(
          "GET",
  ...
  const servicesResume = (orders: Order[]) => {
    ...
    const diffTime = mostRecent.startDate
      ? Math.abs(today.getTime() - new Date(mostRecent.startDate).getTime())
      : 0;
    const daysSinceLast = mostRecent.startDate
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <p className="font-medium">
                          {servicesResume(orders).totalOrders} ordens no total
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="font-medium">
                          {servicesResume(orders).ordersInProgress} ordens em aberto
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="font-medium">
                          {servicesResume(orders).averageServiceTime.toFixed(1)} dias
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {vehiclesInMaintenance.map((vehicle) => (
                    ...
                    >
                      ...
                      {orders.filter(order => order.vehicleId === vehicle.id && 
                        (order.status === "IN_PROGRESS" || order.status === "PENDING")).length > 0 && (
                        <div className="mt-4 pt-4 border-t border-gray-800">
                          ...
                          {orders.filter(order => order.vehicleId === vehicle.id && 
                            ...
                            <div key={order.id} className="text-sm">
                              ...
                              <p className="text-gray-400 mt-1">
                                Início: {formatShortDate(order.startDate)}
                              </p>
                            </div>
                          ))}
                        </div>
                  ...
                  <div className="mt-4">
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <p className="font-medium">
                          {formatCurrency(servicesResume(orders).averageCost)}
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="font-medium">
                          {servicesResume(orders).completedOrders} concluídos
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="font-medium">
                          {servicesResume(orders).ordersInProgress} em aberto
                        </p>
                      </div>
                    </div>
                  </div>
                          ...
                          <div className="space-y-2">
                            <div className="flex justify-between items-center">
                              <span className="text-sm">
                                {vehiclesInMaintenance.length} veículos em manutenção
                              </span>
                            </div>
                            <div className="h-2 bg-gray-800 rounded-full">
                              <div
